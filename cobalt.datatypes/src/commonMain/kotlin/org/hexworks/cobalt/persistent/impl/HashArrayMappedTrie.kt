...
abstract class HAMTNode<K, V> : HashArrayMappedTrie<K, V> {
    ...
    abstract fun lookup(shift: Int, keyHash: Int, key: K): Maybe<out V>
    ...
    override fun iterator(): Iterator<Pair<K, V>> {
        return nodes().map { node -> node.key to node.value }
    }
    ...
    override fun get(key: K): Maybe<out V> {
    ...
    override fun toString() = iterator().asSequence().joinToString()
...
class EmptyNode<K, V> private constructor() : HAMTNode<K, V>() {
    ...
    override fun nodes(): Iterator<LeafNode<K, V>> = Iterator.empty()
...
class LeafSingleton<K, V>(override val hash: Int,
    ...
    override fun nodes(): Iterator<LeafNode<K, V>> = Iterator.of(this)
